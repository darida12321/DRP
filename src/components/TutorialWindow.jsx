...
function TutorialWindow(props) {
  ...
  const completed = useCallback(() => {
    if(!lessonData.examples){ return false }
  ...
  return (
    <div id="tutorial">
      <div id="textbox">
        <div id="lesson-info">
          <h1 id="lesson-title">
            Lesson {lessonData.num}: {lessonData.title}
          </h1>
          <p id="lesson-desc">{
            lessonData.lesson && lessonData.lesson.description
          }</p>
        </div>
        <div id="lesson-marker">
          <div id="blob" style={{ background: completed() ? colorComplete : colorDefault }}>
            {exampleNum}/{
              lessonData.examples && lessonData.examples.length
            }
          </div>
        </div>
      </div>
